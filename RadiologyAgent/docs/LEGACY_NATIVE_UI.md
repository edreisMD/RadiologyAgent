# Legacy native workspace

**Not for medical use, research only.**

This is the earlier optional API-backed interface, available from the Study menu. The default shared Codex workspace does not start its automatic worker. Use the [current app guide](../README.md) for installation.
...
For a shared clinic deployment, see [deployment](DEPLOYMENT.md) and [API integration](API.md). Docker Compose is provided; it binds the published port to loopback. Use a clinic-managed HTTPS reverse proxy for remote workstations.
...
See the [production review and remaining release gates](PRODUCTION_REVIEW.md), [viewer controls](VIEWER.md), [security and data handling](../SECURITY.md), [contributing](../CONTRIBUTING.md), and [third-party notices](../THIRD_PARTY_NOTICES.md).

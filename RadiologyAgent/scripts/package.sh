 ...
 for item in include:
  ...
  for file in (path.rglob('*') if path.is_dir() else [path]):
   if file.is_file() and not any(part in {'__pycache__','.pytest_cache','node_modules','.venv','dist'} for part in file.relative_to(root).parts):
    archive.write(file,'Radiology Agent/'+str(file.relative_to(root)))
 source=root.parent/'RadiologyConnector'
 if source.is_dir():
  for file in source.rglob('*'):
   if file.is_file() and not any(part in {'__pycache__','.pytest_cache','node_modules','.venv','dist'} for part in file.relative_to(source).parts):
